...
/*   Updated: 2022/06/20 16:00:04 by alukongo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_usleep(t_data *data, long time_to)
{
	...
	{
		pthread_mutex_lock(&data->philo->runtime);
		current = (get_time(2) - data->time->r_start) * 1000
			+ ((get_time(0)) - (data->time->r_ustart)) / 1000 - start_time;
		pthread_mutex_unlock(&data->philo->runtime);
		if (current >= time_to)
			break ;
		if (time_to - current >= 1000)
			usleep(100);
		else
			usleep((time_to - current) / 10);
		if (ft_check_state(data) != DEAD && data->philo->dead != DEAD)
			pthread_mutex_unlock(&data->philo->state);
		else
		{
